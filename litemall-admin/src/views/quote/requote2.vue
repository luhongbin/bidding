<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button v-permission="['GET /admin/quoteBill/listBrowser']" class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">报价员查询</el-button>
      <el-button v-permission="['GET /admin/quoteBill/listBrowserCeo']" class="filter-item" type="primary" icon="el-icon-search" @click="getListCeo">全部报价查询</el-button>
      <el-button v-permission="['GET /admin/quoteBill/listBrowserOk']" class="filter-item" type="primary" icon="el-icon-search" @click="getListOK">查询中标商品</el-button>
    ...
    <!-- 查询结果 -->
    <el-dialog :visible.sync="DialogVisiable" title="报价单明细">
    ...
    <el-card v-show="rubberCardVisiable" class="box-card">
      ...
      <el-table v-loading="listLoading" :data="quoteRubberslist" element-loading-text="正在查询中。。。" border fit highlight-current-row>
        ...
        <el-table-column align="center" label="备注" prop="id">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="showDetail(scope.row.id, scope.row.quoteBillModelName)">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-card v-show="electronicCardVisiable" class="box-card">
      ...
      <el-table v-loading="listLoading" :data="quoteElectronicslist" element-loading-text="正在查询中。。。" border fit highlight-current-row>
        ...
        <el-table-column property="isDuty" label="结果">
          <template slot-scope="scope">
            <el-tag :type="scope.row.isCeo ? 'success' : 'error' ">{{ scope.row.isDuty ? '选中' : '未选中' }}</el-tag>
          </template>
        </el-table-column>
...
import { listBrowserOK, find, listBrowser, listBrowserCeo } from '@/api/requote'
