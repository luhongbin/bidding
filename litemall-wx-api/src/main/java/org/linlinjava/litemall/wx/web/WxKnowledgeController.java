...
@Validated
public class WxKnowledgeController {
    private final Log logger = LogFactory.getLog(WxCatalogController.class);

    @Autowired
    private LitemallXmCategoryService categoryService;
    ...
    public Object getFirstCategory() {
        // 所有一级分类目录
        List<LitemallXmcatalog> l1CatList = categoryService.queryL1();
    ...
    public Object getSecondCategory(@NotNull Integer id) {
        // 所有二级分类目录
        List<LitemallXmcatalog> currentSubCategory = categoryService.queryByPid(id);
    ...
    public Object index(Integer id) {
        ...
        // 所有一级分类目录
        List<LitemallXmcatalog> l1CatList = categoryService.queryL1();
        List<LitemallKnowledge> hotknowledge = knowledgeService.queryByIsPopular(0, 10);

        // 当前一级分类目录
        LitemallXmcatalog currentCategory = null;
        if (id != null) {
        ...
        // 当前一级分类目录对应的二级分类目录
        List<LitemallXmcatalog> currentSubCategory = null;
        if (null != currentCategory) {
            currentSubCategory = categoryService.queryByPid(currentCategory.getId());
        }

        Map<String, Object> data = new HashMap<String, Object>();
        data.put("hotknowledgeList", hotknowledge);
    ...
    public Object queryAll() {
        ...
        //所有子分类列表
        Map<Integer, List<LitemallXmcatalog>> allList = new HashMap<>();
        List<LitemallXmcatalog> sub;
        ...
        // 当前一级分类目录
        LitemallXmcatalog currentCategory = l1CatList.get(0);
    ...
    public Object current(@NotNull Integer id) {
        // 当前分类
        LitemallXmcatalog currentCategory = categoryService.findById(id);
        if(currentCategory == null){
            return ResponseUtil.badArgumentValue();
        }
        List<LitemallXmcatalog> currentSubCategory = categoryService.queryByPid(currentCategory.getId());
    ...
    @GetMapping("sub")
    public Object sub(@NotNull Integer id) {
        LitemallXmcatalog currentCategory = categoryService.findById(id);

        List<LitemallXmcatalog> SubCategory = categoryService.queryByPid(currentCategory.getId());
    ...
    public Object list(@NotNull Integer id) {
        // 当前分类
        List<LitemallKnowledge> knowledgeCategory = knowledgeService.queryByCategory(id,0,100);

        Map<String, Object> data = new HashMap<String, Object>();
        data.put("knowCategory", knowledgeCategory);
        return ResponseUtil.ok(data);
    }
    @GetMapping("detail")
    public Object detail(@NotNull Integer id) {
        // 当前分类
        LitemallKnowledge info = knowledgeService.findById(id);

        Map<String, Object> data = new HashMap<String, Object>();
        data.put("knowInfo", info);
        return ResponseUtil.ok(data);
    }
}
